...
import { Container, ComponentWrapper } from 'lib/components'
  ...
  return (
    <ComponentWrapper bg="gray.100">
      ...
      <Container maxW="5xl" mt="-120px">
        <Stack spacing={[6, '12']} bg="white" p={[3, 10]} py={[4, 16]} boxShadow="2xl" borderRadius="30px" {...props}>
          <Box mx="auto">
...
const RegisterForm = (props: any) => {
  const { register, handleSubmit, errors, formState, triggerValidation, getValues, reset } = useForm({
    defaultValues: {
      email: '',
      firstname: '',
      lastname: '',
  ...
  const doValidate = (e: any) => triggerValidation(e.target.name)
  return (
    <Stack as="form" onSubmit={handleSubmit(onSubmit)} spacing="6" maxW="lg" width="full" {...props}>
      <FormInput
...
const Checkbox = React.forwardRef((props: any, ref) => {
  const { isInvalid, touched, ...rest } = props
  return (
    <BaseCheckbox ref={ref} size="lg" variantColor="gray" borderColor={!isInvalid ? 'gray.300' : 'red.300'} {...rest}>
      <Text fontSize="sm">
        Accept{' '}
...
const FormInput = React.forwardRef((props: any, ref: any) => {
  const { touched, isInvalid, ...rest } = props
  ...
  return (
    <InputGroup size="lg">
      ...
      <InputRightElement height={[12, 16]} pr="4">
        {touched && <Icon name={!isInvalid ? 'check-circle' : 'warning'} size="8" color={!isInvalid ? 'purple.300' : 'red.300'} />}
      </InputRightElement>
    </InputGroup>
  )
})

const api = async ({ delay, data }: any) => {
